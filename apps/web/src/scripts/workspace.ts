...
registerAllModules();
const forceX=(d3Force3d as any).forceX as (x?:number|((node:any)=>number))=>any;
...
type Activity = {
  ...
  referenceId?: string;
  mapReady?: boolean;
  channel?: 'process' | 'openalex' | 'graph';
  raw?: string;
  updatedAt: number;
};
...
const CONSOLE_HEIGHT_KEY = 'seshat.workspace.console-height.v1';
...
export function mountSeshatWorkspace(root: HTMLElement): void {
  ...
  const renderActivities = () => {
    const keepPinnedToLatest = consoleDrawer.hidden || consoleDrawer.scrollHeight - consoleDrawer.scrollTop - consoleDrawer.clientHeight < 36;
    const active = activities.filter((activity) => activity.state === 'working');
    const ordered = [...activities].sort((left, right) => left.updatedAt - right.updatedAt);
    ...
    ordered.slice(-300).forEach((activity) => {
      ...
      const header = document.createElement('div'); header.className = 'console-entry-header';
      const timestamp = document.createElement('time'); timestamp.dateTime = new Date(activity.updatedAt).toISOString(); timestamp.textContent = new Date(activity.updatedAt).toLocaleTimeString([], { hour12: false });
      const channel = document.createElement('span'); channel.className = 'console-channel'; channel.textContent = activity.channel || 'process';
      const message = document.createElement('span'); message.className = 'console-message'; message.textContent = activity.message;
      header.append(timestamp, channel, message); entry.appendChild(header);
      if (activity.raw) { const raw = document.createElement('pre'); raw.textContent = activity.raw; entry.appendChild(raw); }
      item.append(mark, entry);
    ...
    if (keepPinnedToLatest && !consoleDrawer.hidden) window.requestAnimationFrame(() => { consoleDrawer.scrollTop = consoleDrawer.scrollHeight; });
  };
  ...
  const updateActivity = (id: string, patch: Partial<Activity>) => {
    ...
    else activities.push({ id, message: patch.message || 'Working…', state: patch.state || 'working', referenceId: patch.referenceId, mapReady: patch.mapReady, channel: patch.channel || 'process', raw: patch.raw, updatedAt: Date.now() });
    if (activities.length > 300) activities.splice(0, activities.length - 300);
    renderActivities();
  };

  let consoleTraceSequence = 0;
  const consoleRaw = (value: unknown): string => {
    if (typeof value === 'string') return value;
    try { const serialized = JSON.stringify(value, null, 2); return serialized === undefined ? String(value) : serialized; }
    catch { return String(value); }
  };
  const appendConsoleTrace = (channel: 'openalex' | 'graph', message: string, raw: unknown, state: Activity['state'] = 'complete') => {
    activities.push({ id: `trace-${Date.now()}-${consoleTraceSequence++}`, message, state, channel, raw: consoleRaw(raw), updatedAt: Date.now() });
    if (activities.length > 300) activities.splice(0, activities.length - 300);
    renderActivities();
  };
  const consoleRequestBody = (body: BodyInit | null | undefined) => {
    if (typeof body !== 'string') return body ? `[${body.constructor?.name || 'Body'}]` : undefined;
    try { return JSON.parse(body); } catch { return body; }
  };
  const nativeFetch = window.fetch.bind(window);
  const fetchConsoleJson = async (channel: 'openalex' | 'graph', label: string, url: string, init: RequestInit = {}) => {
    const method = String(init.method || 'GET').toUpperCase();
    const request = { method, url, headers: init.headers || {}, body: consoleRequestBody(init.body) };
    appendConsoleTrace(channel, `→ ${method} ${label}`, request);
    const startedAt = performance.now();
    try {
      const response = await nativeFetch(url, init);
      const rawText = await response.clone().text();
      let body: unknown = rawText;
      try { body = rawText ? JSON.parse(rawText) : null; } catch { /* Preserve non-JSON responses verbatim. */ }
      appendConsoleTrace(channel, `← ${response.status} ${label} · ${Math.round(performance.now() - startedAt)} ms`, {
      ...
      appendConsoleTrace(channel, `× NETWORK ${label} · ${Math.round(performance.now() - startedAt)} ms`, {
        request,
  ...
  const traceableRequest = (url: string, init: RequestInit) => {
    if (url.includes('/api/papers/')) return { channel: 'openalex' as const, label: url.split('?')[0] };
    if (url.includes('/api/knowledge-graph/neighborhood') || url.includes('/api/knowledge-graph/expand')) return { channel: 'openalex' as const, label: url.split('?')[0] };
    if (url.includes('/api/knowledge-graph')) return { channel: 'graph' as const, label: url.split('?')[0] };
    if (url.includes('/api/library/actions') && consoleRaw(consoleRequestBody(init.body)).includes('scholarly')) return { channel: 'openalex' as const, label: '/api/library/actions' };
    return null;
  };
  window.fetch = (async (input: RequestInfo | URL, init: RequestInit = {}) => {
    const url = typeof input === 'string' ? input : input instanceof URL ? input.toString() : input.url;
    const trace = traceableRequest(url, init);
    if (!trace) return nativeFetch(input, init);
    const method = String(init.method || (input instanceof Request ? input.method : 'GET')).toUpperCase();
    const request = { method, url, headers: init.headers || (input instanceof Request ? Object.fromEntries(input.headers.entries()) : {}), body: consoleRequestBody(init.body) };
    appendConsoleTrace(trace.channel, `→ ${method} ${trace.label}`, request);
    const startedAt = performance.now();
    try {
      ...
      let body: unknown = rawText;
      try { body = rawText ? JSON.parse(rawText) : null; } catch { /* Preserve non-JSON responses verbatim. */ }
      appendConsoleTrace(trace.channel, `← ${response.status} ${trace.label} · ${Math.round(performance.now() - startedAt)} ms`, {
        status: response.status,
        statusText: response.statusText,
        headers: Object.fromEntries(response.headers.entries()),
        body,
      ...
      appendConsoleTrace(trace.channel, `× NETWORK ${trace.label} · ${Math.round(performance.now() - startedAt)} ms`, {
        request,
        error: error instanceof Error ? { name: error.name, message: error.message, stack: error.stack } : String(error),
      }, 'error');
      throw error;
  ...
  const runReferenceAction = async (ids: string[], action: 'reprocess-metadata' | 'scholarly' | 'summarize' | 'extract' | 'relate') => {
    ...
    const { response, data: result } = action === 'scholarly'
      ? await fetchConsoleJson('openalex', 'queue paper enrichment', '/api/library/actions', actionRequest)
      : await (async () => { const next = await fetch('/api/library/actions', actionRequest); return { response: next, data: await next.json().catch(() => ({})) }; })();
  ...
  const openChirpNarration = (reference:ReferenceRow) => {
    ...
    const languageLabel=document.createElement('label');languageLabel.textContent='Language';const language=document.createElement('select');
    [['es','Español'],['en','English'],['de','Deutsch'],['nb','Norsk bokmål'],['fr','Français'],['it','Italiano']].forEach(([value,label])=>{const option=document.createElement('option');option.value=value;option.textContent=label;option.selected=normalizeReaderLanguage(reference.language||navigator.language)===value;language.appendChild(option);});languageLabel.appendChild(language);
    const voiceLabel=document.createElement('label');voiceLabel.textContent='Voice';const voice=document.createElement('select');voiceLabel.appendChild(voice);
  ...
  const referenceMenuItems = (ids: string[], requestedCollectionId: string | null = activeLibrary) => {
    const editableIds = ids.filter((id) => references.get(id)?.access !== 'viewer');
    const enrichLabel=editableIds.length===1?'Enrich paper with OpenAlex':`Enrich ${editableIds.length} papers with OpenAlex`;
    ...
    { label: enrichLabel, disabled: !editableIds.length, action: () => runReferenceAction(editableIds, 'scholarly') },
    ...
    return { element, init() {
      ...
      if (kind === 'graph') {
        ...
        const conceptSidebar=document.createElement('div'); conceptSidebar.className='graph-concepts'; conceptsSection.content.appendChild(conceptSidebar);
	        const selectionSection=section('Selection',true);
	        const selectionList=document.createElement('div');selectionList.className='graph-selection-list';
	        const inspector=document.createElement('div'); inspector.className='graph-inspector'; inspector.innerHTML='<p>Select a paper or association to inspect its evidence.</p>'; selectionSection.content.append(selectionList,inspector);
	        const abstractSection=section('Abstract',false);abstractSection.badge.remove();
	        const abstractSidebar=document.createElement('div');abstractSidebar.className='graph-abstract';abstractSidebar.innerHTML='<p>Select a paper to read its abstract.</p>';abstractSection.content.appendChild(abstractSidebar);
	        const referencesSection=section('Refs',true);
	        const referenceSidebar=document.createElement('div');referenceSidebar.className='graph-reference-list';referenceSidebar.innerHTML='<p>Select a paper to inspect its references.</p>';referencesSection.content.appendChild(referenceSidebar);
	        const citationsSection=section('Cited by',false);
        const citationSidebar=document.createElement('div');citationSidebar.className='graph-reference-list';citationSidebar.innerHTML='<p>Open this section to find papers that cite the selection.</p>';citationsSection.content.appendChild(citationSidebar);
        const similarSection=section('Similar',false);
        const similarSidebar=document.createElement('div');similarSidebar.className='graph-reference-list';similarSidebar.innerHTML='<p>Open this section to find related and structurally similar papers.</p>';similarSection.content.appendChild(similarSidebar);
        ...
        const url = referenceId ? `/api/knowledge-graph?paperId=${encodeURIComponent(referenceId)}` : '/api/knowledge-graph/corpus';
        void fetch(url).then((response) => response.json()).then((data) => {
          ...
          const enabled: Record<string,boolean> = { paper:!globalGraph,author:!globalGraph,topic:true,emergence:false,venue:true,institution:true,collection:true };
          const enabledEdges: Record<string,boolean> = { cites:true,'related-to':true,'bibliographic-coupling':true,'co-citation':true,'shared-author':true,'shared-topic':true,'authored-by':true,'has-topic':true,'published-in':true,'affiliated-with':true,'belongs-to-collection':true,'concept-related':true,'concept-author':true,'concept-emergence':true };
          let neighborDepth = 0; let folding = false; let minimumWeight = 0; let graphQuery = ''; let layoutMode: 'relevance' | 'concepts' = 'relevance';const selectedNodeIds=new Set<string>();const discoveryRequests:Record<'references'|'citations'|'similar',number>={references:0,citations:0,similar:0};let activeDiscoveryNode:any=null;let hierarchyRootId:string|null=null;let hoveredNodeId:string|null=null;let hoveredBranchIds=new Set<string>();let stability=.7;let showConceptClouds=true;let renderedNodes:any[]=[];let renderedLinks:any[]=[];
          ...
          const authorKinds=new Set(['person','author','editor','composer','performer']);
          const emergenceKinds=new Set(['emergence','emerging-language']);
          const nodeKind=(node:any)=>normalize(node.kind||node.classification).replaceAll('_','-');
          let degreeCache=new Map<string,number>();let neighborCache=new Map<string,string[]>();let nodeCache=new Map<string,any>();let paperAuthorCache=new Map<string,string[]>();let repulsion:number=GRAPH_LAYOUT_DEFAULTS.repulsion;let linkDistance:number=GRAPH_LAYOUT_DEFAULTS.distance;
          const refreshDegrees=()=>{degreeCache=new Map<string,number>();neighborCache=new Map<string,string[]>();nodeCache=new Map(allNodes.map((node:any)=>[String(node.id),node]));paperAuthorCache=new Map<string,string[]>();allLinks.forEach((link:any)=>{const source=sourceId(link),target=targetId(link),relation=String(link.relation||link.kind||'').toLowerCase();degreeCache.set(source,(degreeCache.get(source)||0)+1);degreeCache.set(target,(degreeCache.get(target)||0)+1);neighborCache.set(source,[...(neighborCache.get(source)||[]),target]);neighborCache.set(target,[...(neighborCache.get(target)||[]),source]);if(relation==='authored-by'){const author=nodeCache.get(target);if(author)paperAuthorCache.set(source,[...(paperAuthorCache.get(source)||[]),String(author.label||'')].filter(Boolean));}});};
          const nodeRelevance=(node:any)=>Math.log1p(Number(node.properties?.citedByCount||node.properties?.count||node.count||0))+Math.sqrt(degreeCache.get(String(node.id))||0)+Number(node.properties?.score||node.properties?.strength||0)*2;
          const nodeRadius=(node:any)=>Math.max(4,Math.min(18,Number(node.properties?.radius)||5+nodeRelevance(node)*.45));
          const paperMeta=(node:any)=>{const local=references.get(String(node.properties?.referenceId||''));const authors=local?.contributors.filter((person)=>person.role==='author').map((person)=>person.family||person.literal||[person.given,person.family].filter(Boolean).join(' ')).filter(Boolean);const fallback=local?.contributorsDisplay||paperAuthorCache.get(String(node.id))||node.properties?.authors||node.properties?.author;const author=compactGraphAuthor(authors?.length?authors:fallback);const year=String(node.properties?.year||local?.year||'n.d.');return{author,year};};
          const nodeLabelLines=(node:any)=>{const label=plainInlineTitle(node.label||'');if(!paperKinds.has(nodeKind(node)))return wrapGraphLabel(label,emergenceKinds.has(nodeKind(node))?28:24,3);const {author,year}=paperMeta(node);if(hoveredNodeId===String(node.id))return[...wrapGraphLabel(label,30,8),`${author} · ${year}`];return[author,year];};
          const mergeGraphData=(nodes:any[],edges:any[])=>{const nodeMap=new Map(allNodes.map((node:any)=>[String(node.id),node]));nodes.forEach((node:any)=>{const current=nodeMap.get(String(node.id));const incoming={...node,properties:{...(node.properties||{})}};nodeMap.set(String(node.id),current?{...current,...incoming,properties:{...(current.properties||{}),...incoming.properties}}:incoming);});const edgeMap=new Map(allLinks.map((edge:any)=>[String(edge.id),edge]));edges.forEach((edge:any)=>edgeMap.set(String(edge.id),{...edge,source:sourceId(edge),target:targetId(edge),properties:{...(edge.properties||{})}}));allNodes.splice(0,allNodes.length,...nodeMap.values());allLinks.splice(0,allLinks.length,...edgeMap.values());};
          const seedExpansion=(rootId:string,nodes:any[],mode:'references'|'citations'|'similar')=>{const rootNode=allNodes.find((candidate:any)=>String(candidate.id)===rootId),known=new Set(allNodes.map((candidate:any)=>String(candidate.id))),fresh=nodes.filter((candidate:any)=>!known.has(String(candidate.id)));const originX=Number(rootNode?.x)||0,originY=Number(rootNode?.y)||0,direction=mode==='references'?1:mode==='citations'?-1:0;fresh.forEach((node:any,index:number)=>{const slot=index%8,ring=Math.floor(index/8)+1,spread=(slot/7-.5)*Math.PI*.9,angle=direction?direction*Math.PI/2+spread:(index/Math.max(1,fresh.length))*Math.PI*2,radius=Math.min(420,linkDistance*.65+ring*48);node.x=originX+Math.cos(angle)*radius;node.y=originY+Math.sin(angle)*radius;node.vx=0;node.vy=0;});allNodes.forEach((node:any)=>{node.vx=0;node.vy=0;});hierarchyRootId=rootId;};
          const addLocalKeywordSeeds=(nextReferenceId:string)=>{const local=references.get(nextReferenceId);if(!local)return;let rootNode=allNodes.find((node:any)=>String(node.properties?.referenceId||'')===nextReferenceId);if(!rootNode){rootNode={id:`paper:local:${nextReferenceId}`,kind:'paper',label:local.title,properties:{referenceId:nextReferenceId,local:true,referenceCount:0}};allNodes.push(rootNode);}const nodes:any[]=[];const edges:any[]=[];for(const keyword of local.keywords){const normalized=normalizeGraphKeyword(keyword);if(!normalized)continue;let concept=allNodes.find((node:any)=>conceptKinds.has(nodeKind(node))&&normalizeGraphKeyword(node.label)===normalized);if(!concept){concept={id:`concept:keyword:${encodeURIComponent(normalized).slice(0,180)}`,kind:'concept',label:keyword,properties:{localKeyword:true,keyword}};nodes.push(concept);}edges.push({id:`keyword:${nextReferenceId}:${encodeURIComponent(normalized)}`,source:String(rootNode.id),target:String(concept.id),relation:'has-topic',weight:1,properties:{directed:true,evidence:{method:'vault-keyword'},provenance:{source:'catalog'}}});}mergeGraphData(nodes,edges);};
          if(referenceId)addLocalKeywordSeeds(referenceId);
          const applyLayout=(reheat=true)=>{if(!graph)return;const hierarchy=hierarchyRootId?citationHierarchyDepths(hierarchyRootId,allLinks):new Map<string,number>();graph.d3VelocityDecay(stability);graph.d3AlphaDecay(.075);graph.d3Force('charge')?.strength((node:any)=>-Math.min(GRAPH_LAYOUT_DEFAULTS.maximumRepulsion,repulsion+nodeRelevance(node)*28));graph.d3Force('link')?.distance((link:any)=>Math.max(70,linkDistance-Number(link.weight||0)*12));const collision:any=forceCollide((node:any)=>graphLabelCollisionRadius(nodeLabelLines(node),nodeRadius(node)));graph.d3Force('collide',collision.strength(1).iterations(2));if(globalGraph){graph.d3Force('radial',null);graph.d3Force('x',forceX((node:any)=>authorKinds.has(nodeKind(node))?-230:paperKinds.has(nodeKind(node))?230:0).strength((node:any)=>conceptKinds.has(nodeKind(node))?.02:.075));graph.d3Force('y',forceY((node:any)=>emergenceKinds.has(nodeKind(node))?-210:authorKinds.has(nodeKind(node))?150:paperKinds.has(nodeKind(node))?150:0).strength((node:any)=>conceptKinds.has(nodeKind(node))?.02:.075));}else if(layoutMode==='concepts'){graph.d3Force('x',forceX(0).strength(.025));graph.d3Force('y',forceY((node:any)=>conceptKinds.has(nodeKind(node))?-100:paperKinds.has(nodeKind(node))?90:220).strength((node:any)=>conceptKinds.has(nodeKind(node))?.28:.1));graph.d3Force('radial',forceRadial((node:any)=>conceptKinds.has(nodeKind(node))?Math.max(20,170-nodeRelevance(node)*10):paperKinds.has(nodeKind(node))?250:360,0,0).strength(.09));}else{const gap=Math.min(180,Math.max(110,linkDistance*.85));graph.d3Force('radial',null);graph.d3Force('x',forceX(0).strength((node:any)=>hierarchy.has(String(node.id)) ? .055 : .012));graph.d3Force('y',forceY((node:any)=>{const depth=hierarchy.get(String(node.id));if(depth!==undefined)return depth*gap;return paperKinds.has(nodeKind(node))?-Math.min(120,nodeRelevance(node)*12):conceptKinds.has(nodeKind(node))?120:210;}).strength((node:any)=>hierarchy.has(String(node.id)) ? .34 : .065));}if(reheat)graph.d3ReheatSimulation();};
          const visibleData = () => {
          ...
          const renderSelection=()=>{selectionSection.badge.textContent=String(selectedNodeIds.size);selectionList.replaceChildren();if(!selectedNodeIds.size){const empty=document.createElement('p');empty.textContent='Click a node · Shift/⌘ click for multiple selection.';selectionList.appendChild(empty);return;}const clear=document.createElement('button');clear.type='button';clear.className='graph-selection-clear';clear.textContent='Clear selection';clear.onclick=()=>{selectedNodeIds.clear();renderSelection();graph?.refresh?.();};selectionList.appendChild(clear);for(const id of selectedNodeIds){const node=allNodes.find((candidate:any)=>String(candidate.id)===id);if(!node)continue;const item=document.createElement('button');item.type='button';item.className='graph-selection-item';setInlineTitle(item,node.label||id);item.title=plainInlineTitle(node.label||id);item.onclick=()=>{selectGraphNode(node);graph.centerAt(Number(node.x)||0,Number(node.y)||0,300);};selectionList.appendChild(item);}};
          const renderConcepts=()=>{const concepts=allNodes.filter((node:any)=>conceptKinds.has(nodeKind(node))).map((node:any)=>({node,count:Number(node.properties?.count||node.count||0)||degreeCache.get(String(node.id))||0})).sort((left,right)=>right.count-left.count||String(left.node.label).localeCompare(String(right.node.label)));conceptsSection.badge.textContent=String(concepts.length);conceptSidebar.replaceChildren();const list=document.createElement('div');list.className='graph-concept-list';concepts.forEach(({node,count})=>{const control=document.createElement('button');control.type='button';const label=document.createElement('span');label.textContent=String(node.label||'Untitled concept');const total=document.createElement('b');total.textContent=String(Number(node.properties?.vaultMatchCount||0)||count);control.append(label,total);control.onclick=()=>{layoutMode='concepts';layout.value='concepts';focusId=String(node.id);neighborDepth=Math.max(1,neighborDepth);selectedNodeIds.clear();selectedNodeIds.add(focusId);renderSelection();void unfoldConcept(node);graph.centerAt(Number(node.x)||0,Number(node.y)||0,400);graph.zoom(2.2,400);};list.appendChild(control);});conceptSidebar.appendChild(list);};
          const update = () => { refreshDegrees();if(hoveredNodeId)hoveredBranchIds=new Set([hoveredNodeId,...(neighborCache.get(hoveredNodeId)||[])]);const next = visibleData();renderedNodes=next.nodes;renderedLinks=next.links; count.textContent = `${next.nodes.length} nodes · ${next.links.length} links`; graph.graphData(next); applyLayout(); renderConcepts(); };
          const safe=(value:unknown)=>safeInlineTitleHtml(value);
          ...
          const showAbstract=(value:unknown,open=true)=>{const text=String(value||'').trim();abstractSidebar.replaceChildren();const paragraph=document.createElement('p');paragraph.textContent=text||'No abstract is available for this paper.';abstractSidebar.appendChild(paragraph);if(open)abstractSection.details.open=true;};
          const resetDiscoveryPanels=()=>{for(const mode of ['references','citations','similar'] as const)discoveryRequests[mode]+=1;referencesSection.badge.textContent='';citationsSection.badge.textContent='';similarSection.badge.textContent='';abstractSection.details.open=false;abstractSidebar.innerHTML='<p>Select a paper to read its abstract.</p>';referenceSidebar.innerHTML='<p>Select a paper to inspect its references.</p>';citationSidebar.innerHTML='<p>Open this section to find papers that cite the selection.</p>';similarSidebar.innerHTML='<p>Open this section to find related and structurally similar papers.</p>';};
          const renderDiscovery=async(mode:'references'|'citations'|'similar',node:any,sectionState:ReturnType<typeof section>,sidebar:HTMLElement)=>{const nodeId=String(node.id||''),request=++discoveryRequests[mode],referenceId=String(node.properties?.referenceId||''),openAlexId=String(node.properties?.openAlexId||'');if(mode==='references')sectionState.details.open=true;sidebar.innerHTML=`<p>Loading ${mode}…</p>`;try{const query=new URLSearchParams({nodeId,mode,maximum:'100'});if(referenceId)query.set('referenceId',referenceId);if(openAlexId)query.set('openAlexId',openAlexId);const {response,data}=await fetchConsoleJson('openalex',`paper ${mode}`,`/api/knowledge-graph/neighborhood?${query}`,{cache:'no-store'});if(request!==discoveryRequests[mode])return;if(!response.ok)throw new Error(data.error||`${mode} unavailable.`);const responseNodes=data.nodes||[];const responseEdges=(data.edges||[]).map((edge:any)=>({...edge,source:String(typeof edge.source==='object'?edge.source.id:edge.source),target:String(typeof edge.target==='object'?edge.target.id:edge.target)}));seedExpansion(nodeId,responseNodes,mode);mergeGraphData(responseNodes,responseEdges);node=allNodes.find((candidate:any)=>String(candidate.id)===nodeId)||node;node.properties={...(node.properties||{}),...(mode==='references'?{referenceCount:Number(data.total||0)}:mode==='citations'?{citedByCount:Number(data.total||0)}:{relatedCount:Number(data.total||0)})};const neighborIds=responseEdges.map((edge:any)=>edge.source===nodeId?edge.target:edge.target===nodeId?edge.source:'').filter(Boolean);const paperById=new Map(allNodes.map((candidate:any)=>[String(candidate.id),candidate]));const papers=[...new Map(neighborIds.map((id:string)=>{const paper=paperById.get(id);return paperKinds.has(nodeKind(paper||{}))?[id,paper]:null;}).filter(Boolean) as Array<[string,any]>).values()];update();sectionState.badge.textContent=String(Number(data.total||papers.length));sidebar.replaceChildren();const summary=document.createElement('p');summary.className='graph-reference-summary';const source=String(data.source||'catalog').replaceAll('+',' + ');summary.textContent=`${data.total||papers.length} ${mode}${data.truncated?` · showing ${papers.length}`:''} · ${source}${data.warning?' · OpenAlex unavailable':''}`;sidebar.appendChild(summary);if(!papers.length){const empty=document.createElement('p');empty.textContent=mode==='references'?'No parsed or OpenAlex references are available yet.':mode==='citations'?'No citing papers are available yet.':'No similar papers are available yet.';sidebar.appendChild(empty);return;}papers.forEach((paper:any)=>{const control=document.createElement('button');control.type='button';const title=document.createElement('span');setInlineTitle(title,paper.label||'Untitled paper');const meta=document.createElement('small');const year=String(paper.properties?.year||'').trim(),refs=Number(paper.properties?.referenceCount||0),citations=Number(paper.properties?.citedByCount||0);meta.textContent=[year,refs?`${refs} refs`:'',citations?`${citations} citations`:''].filter(Boolean).join(' · ')||String(paper.properties?.openAlexId||'bibliography');control.append(title,meta);control.onclick=()=>selectGraphNode(paper);sidebar.appendChild(control);});}catch(error){if(request!==discoveryRequests[mode])return;sectionState.badge.textContent='!';sidebar.innerHTML=`<p class="graph-error">${safe(error instanceof Error?error.message:`${mode} unavailable.`)}</p>`;}};
          const renderReferences=(node:any)=>renderDiscovery('references',node,referencesSection,referenceSidebar);
          const renderPaper=async(node:any)=>{selectionSection.details.open=true;activeDiscoveryNode=node;resetDiscoveryPanels();const id=String(node.properties?.referenceId||''),localReference=references.get(id),openAlexId=String(node.properties?.openAlexId||'');inspector.innerHTML=`<div class="eyebrow">${safe(node.kind)}</div><h3>${safe(node.label)}</h3><p>${safe(openAlexId||[node.properties?.authors?.join?.(', '),node.properties?.year].filter(Boolean).join(' · ')||'Catalog item')}</p>`;if(globalGraph){if(id){const actions=document.createElement('div');actions.className='graph-inspector-actions';const open=document.createElement('button');open.type='button';open.textContent='Open document';open.onclick=()=>controller.openDocument(id);actions.appendChild(open);inspector.appendChild(actions);}return;}showAbstract(node.properties?.abstract||localReference?.abstract);void renderReferences(node);if(citationsSection.details.open)void renderDiscovery('citations',node,citationsSection,citationSidebar);if(similarSection.details.open)void renderDiscovery('similar',node,similarSection,similarSidebar);if(!id){if(openAlexId)inspector.insertAdjacentHTML('beforeend',`<p><a href="https://openalex.org/${encodeURIComponent(openAlexId)}" target="_blank" rel="noreferrer">Open in OpenAlex ↗</a></p>`);return;}const response=await fetch(`/api/papers/${encodeURIComponent(id)}`);const data=await response.json();if(!response.ok){inspector.insertAdjacentHTML('beforeend',`<p class="graph-error">${safe(data.error||'Paper details unavailable.')}</p>`);return;}const paper=data.paper;const work=(paper.openAlexWork||{}) as any;if(work.id){node.properties={...(node.properties||{}),openAlexId:work.id,citedByCount:work.citedByCount,referenceCount:work.referencedWorkIds?.length,authors:work.authors?.map?.((author:any)=>author.name),abstract:work.abstract,year:work.publicationYear};}showAbstract(work.abstract||data.reference?.abstract||node.properties?.abstract||localReference?.abstract);const workTopics=(Array.isArray(work.topics)?work.topics:[]).slice(0,4).map((topic:any)=>String(topic?.name||'')).filter(Boolean);const workType=String(work.type||data.reference?.type||''),workVenue=String(work.venue?.name||''),workYear=work.publicationYear?String(work.publicationYear):'';inspector.insertAdjacentHTML('beforeend',`<dl><dt>Status</dt><dd>${safe(paper.resolutionStatus)}</dd><dt>Method</dt><dd>${safe(paper.resolutionMethod||'—')}</dd><dt>Confidence</dt><dd>${Math.round(Number(paper.resolutionConfidence||0)*100)}%</dd>${workType?`<dt>Type</dt><dd>${safe(workType)}</dd>`:''}${workTopics.length?`<dt>Topics</dt><dd>${workTopics.map((topic:string)=>safe(topic)).join(' · ')}</dd>`:''}${workVenue?`<dt>Venue</dt><dd>${safe(workVenue)}</dd>`:''}${workYear?`<dt>Year</dt><dd>${safe(workYear)}</dd>`:''}</dl>`);const actions=document.createElement('div');actions.className='graph-inspector-actions';const enrich=document.createElement('button');enrich.textContent='Enrich this paper with OpenAlex';enrich.onclick=async()=>{enrich.disabled=true;const result=await fetch(`/api/papers/${encodeURIComponent(id)}/enrich`,{method:'POST'});enrich.textContent=result.ok?'Queued':'Could not queue';};const expand=document.createElement('button');expand.textContent='Refresh this paper graph';expand.onclick=async()=>{expand.disabled=true;expand.textContent='Refreshing…';const result=await fetch('/api/knowledge-graph/expand',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({paperId:id})});if(result.ok){await loadReferenceGraph(id);expand.textContent='Refreshed';}else{const error=await result.json().catch(()=>({}));expand.textContent=error.error||'Refresh failed';expand.disabled=false;}};actions.append(enrich,expand);inspector.appendChild(actions);};
          citationsSection.details.addEventListener('toggle',()=>{if(citationsSection.details.open&&activeDiscoveryNode)void renderDiscovery('citations',activeDiscoveryNode,citationsSection,citationSidebar);});
          similarSection.details.addEventListener('toggle',()=>{if(similarSection.details.open&&activeDiscoveryNode)void renderDiscovery('similar',activeDiscoveryNode,similarSection,similarSidebar);});
          const normalizedNodeKind=nodeKind;
          const connectedCorpusPapers=(node:any)=>{const ids=new Set((Array.isArray(node.properties?.itemIds)?node.properties.itemIds:[]).map(String));return allNodes.filter((candidate:any)=>paperKinds.has(nodeKind(candidate))&&ids.has(String(candidate.properties?.referenceId||''))).sort((left:any,right:any)=>nodeRelevance(right)-nodeRelevance(left));};
          const renderCorpusGroupNode=(node:any,label:string)=>{selectionSection.details.open=true;activeDiscoveryNode=null;resetDiscoveryPanels();const papers=connectedCorpusPapers(node);inspector.innerHTML=`<div class="eyebrow">${safe(label)}</div><h3>${safe(node.label)}</h3><p>${papers.length} connected item${papers.length===1?'':'s'}</p>`;const list=document.createElement('div');list.className='graph-concept-papers';papers.slice(0,120).forEach((paper:any)=>{const control=document.createElement('button');control.type='button';setInlineTitle(control,paper.label||'Untitled item');control.title=plainInlineTitle(paper.label||'');control.onclick=()=>selectGraphNode(paper);control.ondblclick=()=>{const id=String(paper.properties?.referenceId||'');if(id)controller.openDocument(id);};list.appendChild(control);});inspector.appendChild(list);};
          const renderConceptNode=(node:any)=>{if(globalGraph){renderCorpusGroupNode(node,'Concept');return;}selectionSection.details.open=true;activeDiscoveryNode=null;resetDiscoveryPanels();const nodeId=String(node.id);const paperIds=new Set<string>();allLinks.forEach((link:any)=>{if(sourceId(link)===nodeId)paperIds.add(targetId(link));if(targetId(link)===nodeId)paperIds.add(sourceId(link));});const papers=allNodes.filter((candidate:any)=>paperIds.has(String(candidate.id))&&paperKinds.has(nodeKind(candidate))).sort((left:any,right:any)=>nodeRelevance(right)-nodeRelevance(left));inspector.innerHTML=`<div class="eyebrow">Concept</div><h3>${safe(node.label)}</h3><p>${papers.length} connected paper${papers.length===1?'':'s'}</p>`;const list=document.createElement('div');list.className='graph-concept-papers';papers.forEach((paper:any)=>{const control=document.createElement('button');control.type='button';setInlineTitle(control,paper.label||'Untitled paper');control.title=plainInlineTitle(paper.label||'');control.onclick=()=>selectGraphNode(paper);list.appendChild(control);});inspector.appendChild(list);};
          const unfoldConcept=async(node:any)=>{if(globalGraph){renderConceptNode(node);graph?.refresh?.();return;}const keyword=String(node.properties?.keyword||node.label||'');const matches=referencesSharingKeyword(payload.references,keyword);node.properties={...(node.properties||{}),vaultMatchCount:matches.length};const nodes:any[]=[],edges:any[]=[];for(const reference of matches.slice(0,250)){let paper=allNodes.find((candidate:any)=>String(candidate.properties?.referenceId||'')===reference.id);if(!paper){paper={id:`paper:local:${reference.id}`,kind:'paper',label:reference.title,properties:{referenceId:reference.id,local:true,year:reference.year}};nodes.push(paper);}edges.push({id:`keyword:${reference.id}:${encodeURIComponent(normalizeGraphKeyword(keyword))}`,source:String(paper.id),target:String(node.id),relation:'has-topic',weight:1,properties:{directed:true,evidence:{method:'vault-keyword'},provenance:{source:'catalog'}}});}try{const response=await fetch(`/api/knowledge-graph/neighborhood?nodeId=${encodeURIComponent(String(node.id))}&mode=concept&maximum=100`,{cache:'no-store'});if(response.ok){const data=await response.json();nodes.push(...(data.nodes||[]));edges.push(...(data.edges||[]));}}catch{}mergeGraphData(nodes,edges);update();renderConceptNode(allNodes.find((candidate:any)=>String(candidate.id)===String(node.id))||node);graph?.refresh?.();};
          const selectGraphNode=(node:any,event?:MouseEvent)=>{const id=String(node.id);const multiple=Boolean(event?.shiftKey||event?.metaKey||event?.ctrlKey);if(!multiple)selectedNodeIds.clear();if(multiple&&selectedNodeIds.has(id))selectedNodeIds.delete(id);else selectedNodeIds.add(id);focusId=id;if(paperKinds.has(nodeKind(node)))hierarchyRootId=id;let structureChanged=false;if(folding){if(collapsed.has(id))collapsed.delete(id);else collapsed.add(id);structureChanged=true;}renderSelection();if(structureChanged)update();else graph?.refresh?.();if(!selectedNodeIds.has(id))return;if(conceptKinds.has(nodeKind(node)))void unfoldConcept(node);else if(paperKinds.has(nodeKind(node)))void renderPaper(node);else if(globalGraph&&authorKinds.has(nodeKind(node)))renderCorpusGroupNode(node,'Author');else if(globalGraph&&emergenceKinds.has(nodeKind(node)))renderCorpusGroupNode(node,'Emerging language');else{activeDiscoveryNode=null;resetDiscoveryPanels();}};
          const nodeColor=(node:any)=>{const kind=normalizedNodeKind(node);if(['concept','topic','relatedconcept'].includes(kind))return'#b07a3c';if(['emergence','emerging-language'].includes(kind))return'#a15f78';if(['person','author','editor','composer','performer'].includes(kind))return'#4f8265';if(['paper','work','document','publication','article','ebook'].includes(kind))return'#657c9f';if(['institution','organization','publisher','venue','journal'].includes(kind))return'#8b628d';if(['place','location','geographicregion'].includes(kind))return'#b35f58';if(['collection'].includes(kind))return'#d1a73b';if(['method','instrument','system'].includes(kind))return'#4d8c93';return'#7f837d';};
          ...
          const conceptCloudColor=(node:any)=>{let hash=0;for(const character of String(node.label||node.id||''))hash=(hash*31+character.charCodeAt(0))|0;return `hsl(${Math.abs(hash)%360} 31% 49%)`;};
          const conceptCloudPath=(ctx:CanvasRenderingContext2D,cx:number,cy:number,rx:number,ry:number,index:number,scale=1)=>{ctx.beginPath();if(index<=.52){ctx.ellipse(cx,cy,rx*scale,ry*scale,0,0,Math.PI*2);return;}const points=12,angularity=Math.min(.18,(index-.5)*.4);for(let point=0;point<points;point+=1){const angle=-Math.PI/2+point*Math.PI*2/points,spike=point%2===0?1+angularity:1-angularity*.65,x=cx+Math.cos(angle)*rx*scale*spike,y=cy+Math.sin(angle)*ry*scale*spike;if(point===0)ctx.moveTo(x,y);else ctx.lineTo(x,y);}ctx.closePath();};
          const drawConceptClouds=(ctx:CanvasRenderingContext2D,scale:number)=>{if(!showConceptClouds||!renderedNodes.length)return;const visibleById=new Map(renderedNodes.map((node:any)=>[String(node.id),node])),membersByConcept=new Map<string,Map<string,any>>();for(const concept of renderedNodes.filter((node:any)=>conceptKinds.has(nodeKind(node))))membersByConcept.set(String(concept.id),new Map([[String(concept.id),concept]]));for(const link of renderedLinks){const source=sourceId(link),target=targetId(link),sourceNode=visibleById.get(source),targetNode=visibleById.get(target);if(sourceNode&&conceptKinds.has(nodeKind(sourceNode))&&targetNode&&paperKinds.has(nodeKind(targetNode)))membersByConcept.get(source)?.set(target,targetNode);if(targetNode&&conceptKinds.has(nodeKind(targetNode))&&sourceNode&&paperKinds.has(nodeKind(sourceNode)))membersByConcept.get(target)?.set(source,sourceNode);}for(const [conceptId,members] of membersByConcept){const concept=visibleById.get(conceptId),positions=[...members.values()].map((node:any)=>({x:Number(node.x),y:Number(node.y)})).filter((position)=>Number.isFinite(position.x)&&Number.isFinite(position.y));if(!concept||positions.length<2)continue;const minX=Math.min(...positions.map((position)=>position.x)),maxX=Math.max(...positions.map((position)=>position.x)),minY=Math.min(...positions.map((position)=>position.y)),maxY=Math.max(...positions.map((position)=>position.y)),cx=(minX+maxX)/2,cy=(minY+maxY)/2,padding=Math.max(28,46/Math.max(.55,scale)),rx=Math.max(48,(maxX-minX)*.63+padding),ry=Math.max(38,(maxY-minY)*.63+padding),index=conceptKikiBoubaIndex(concept.label,concept.properties||{}),color=conceptCloudColor(concept);ctx.save();conceptCloudPath(ctx,cx,cy,rx,ry,index);ctx.globalAlpha=.13;ctx.fillStyle=color;ctx.fill();ctx.globalAlpha=.3;ctx.strokeStyle=color;ctx.lineWidth=Math.max(.7,1.1/Math.max(.5,scale));ctx.stroke();conceptCloudPath(ctx,cx,cy,rx,ry,index,.72);ctx.globalAlpha=.18;ctx.setLineDash([Math.max(2,4/Math.max(.5,scale)),Math.max(2,5/Math.max(.5,scale))]);ctx.stroke();ctx.setLineDash([]);if(scale>.42){ctx.globalAlpha=.38;ctx.fillStyle=color;ctx.font=`600 ${Math.max(7,9/Math.max(.8,scale))}px "Helvetica Neue",Helvetica,Arial,sans-serif`;ctx.textAlign='center';ctx.textBaseline='bottom';ctx.fillText(plainInlineTitle(concept.label||''),cx,cy-ry+Math.max(10,13/Math.max(.7,scale)));}ctx.restore();}};
          const drawNode=(node:any,ctx:CanvasRenderingContext2D,scale:number)=>{
            const id=String(node.id),x=Number(node.x)||0,y=Number(node.y)||0,kind=normalizedNodeKind(node),radius=nodeRadius(node),selected=selectedNodeIds.has(id),branchActive=!hoveredNodeId||hoveredBranchIds.has(id),hovered=hoveredNodeId===id;
            ctx.save();ctx.globalAlpha=branchActive?1:.12;
            if(selected||hovered){ctx.shadowColor=selected?'#d48738':'#4f8265';ctx.shadowBlur=Math.max(8,13/Math.max(scale,.4));}
            ctx.beginPath();
            if(['concept','topic','relatedconcept'].includes(kind))ctx.rect(x-radius,y-radius,radius*2,radius*2);
            else if(['paper','work','document','publication','article','ebook'].includes(kind))polygon(ctx,x,y,radius*1.25,4,0);
            else if(['institution','organization','publisher','venue','journal'].includes(kind))polygon(ctx,x,y,radius*1.15,6);
            else if(['place','location','geographicregion'].includes(kind))polygon(ctx,x,y,radius*1.2,3);
            else if(['method','instrument','system'].includes(kind))polygon(ctx,x,y,radius*1.15,5);
            else ctx.arc(x,y,radius,0,Math.PI*2);
            ctx.fillStyle=nodeColor(node);ctx.fill();ctx.shadowBlur=0;ctx.lineWidth=Math.max(.7,1/Math.max(scale,.2));ctx.strokeStyle='rgba(20,30,25,.72)';ctx.stroke();
            if(selected||hovered){ctx.beginPath();ctx.arc(x,y,radius+5,0,Math.PI*2);ctx.lineWidth=Math.max(2.2,3/Math.max(scale,.3));ctx.strokeStyle=selected?'#d48738':'#4f8265';ctx.stroke();}
            if(kind==='collection'){ctx.beginPath();ctx.arc(x,y,radius*.55,0,Math.PI*2);ctx.stroke();}
            const lines=nodeLabelLines(node);if(!lines.length){ctx.restore();return;}
            let vectorX=0,vectorY=0;for(const neighborId of neighborCache.get(id)||[]){const neighbor=nodeCache.get(neighborId);if(!neighbor)continue;const dx=Number(neighbor.x)-x,dy=Number(neighbor.y)-y,length=Math.hypot(dx,dy)||1;vectorX+=dx/length;vectorY+=dy/length;}
            const placement=graphLabelPlacement(vectorX,vectorY),fontSize=Math.max(7,Math.min(11,10/Math.max(.75,scale))),lineHeight=fontSize*1.18,paddingX=Math.max(2,3/Math.max(.75,scale)),paddingY=Math.max(2,2.5/Math.max(.75,scale));ctx.font=`500 ${fontSize}px "Helvetica Neue",Helvetica,Arial,sans-serif`;
            const width=Math.max(...lines.map((line)=>ctx.measureText(line).width))+paddingX*2,height=lines.length*lineHeight+paddingY*2,gap=Math.max(4,5/Math.max(.75,scale));let boxX=x-width/2,boxY=y+radius+gap;if(placement==='above')boxY=y-radius-gap-height;else if(placement==='left'){boxX=x-radius-gap-width;boxY=y-height/2;}else if(placement==='right'){boxX=x+radius+gap;boxY=y-height/2;}
            const styles=getComputedStyle(document.documentElement),paper=styles.getPropertyValue('--paper').trim()||'#f1efe6',ink=styles.getPropertyValue('--ink').trim()||'#263129',line=styles.getPropertyValue('--line').trim()||'rgba(20,30,25,.22)';ctx.globalAlpha=branchActive ? .96 : .1;ctx.fillStyle=paper;ctx.fillRect(boxX,boxY,width,height);ctx.globalAlpha=branchActive ? 1 : .12;ctx.strokeStyle=selected?'#d48738':hovered?'#4f8265':line;ctx.lineWidth=selected||hovered?Math.max(1.6,2/Math.max(scale,.3)):Math.max(.45,.7/Math.max(scale,.3));ctx.strokeRect(boxX,boxY,width,height);ctx.fillStyle=ink;ctx.textAlign='center';ctx.textBaseline='middle';lines.forEach((text,index)=>ctx.fillText(text,boxX+width/2,boxY+paddingY+lineHeight*(index+.5)));ctx.restore();
          };
          const hoverGraphNode=(node:any|null)=>{hoveredNodeId=node?String(node.id):null;hoveredBranchIds=hoveredNodeId?new Set([hoveredNodeId,...(neighborCache.get(hoveredNodeId)||[])]):new Set();stage.style.cursor=node?'pointer':'';graph?.refresh?.();};
          graph = new ForceGraph(stage)
            .backgroundColor('rgba(0,0,0,0)')
            .nodeId('id')
            .nodeLabel((node:any)=>{if(!paperKinds.has(nodeKind(node)))return safe(node.label||'');const meta=paperMeta(node);return `<strong>${safe(node.label||'Untitled paper')}</strong><br>${safe(meta.author)} · ${safe(meta.year)}`;})
            .nodeRelSize(5)
            .linkColor((link:any)=>!hoveredNodeId?'rgba(110,120,115,.34)':sourceId(link)===hoveredNodeId||targetId(link)===hoveredNodeId?'rgba(79,130,101,.92)':'rgba(110,120,115,.07)')
            .linkWidth((link:any)=>{const base=Math.max(.35,Math.min(1.2,.4+Number(link.weight||0)*.35));return hoveredNodeId&&(sourceId(link)===hoveredNodeId||targetId(link)===hoveredNodeId)?Math.max(2,base*2):hoveredNodeId ? .2 : base;})
            .linkDirectionalArrowLength((link:any)=>String(link.relation||link.kind)==='cites'?3:0)
            .linkDirectionalArrowRelPos(1)
            .nodeCanvasObjectMode(()=>'replace')
            .nodeCanvasObject(drawNode)
            .onRenderFramePre(drawConceptClouds)
            .onNodeHover(hoverGraphNode)
            .onNodeClick((node:any,event:MouseEvent)=>selectGraphNode(node,event))
            .onBackgroundClick(()=>{selectedNodeIds.clear();focusId=null;hierarchyRootId=null;activeDiscoveryNode=null;renderSelection();resetDiscoveryPanels();inspector.innerHTML='<p>Select a paper or association to inspect its evidence.</p>';if(neighborDepth>0)update();else graph?.refresh?.();})
            .onLinkClick(async(link:any)=>{const edgeId=String(link.id||'');if(!edgeId)return;selectionSection.details.open=true;if(globalGraph){const source=nodeCache.get(sourceId(link)),target=nodeCache.get(targetId(link)),itemIds=Array.isArray(link.properties?.itemIds)?link.properties.itemIds:[];inspector.innerHTML=`<div class="eyebrow">Corpus association</div><h3>${safe(source?.label||sourceId(link))} → ${safe(target?.label||targetId(link))}</h3><dl><dt>Relation</dt><dd>${safe(link.relation||link.kind||'related')}</dd><dt>Shared items</dt><dd>${Number(link.weight||itemIds.length||0)}</dd><dt>Source</dt><dd>Catalog corpus</dd></dl>`;return;}inspector.innerHTML='<p>Loading evidence…</p>';const response=await fetch(`/api/knowledge-graph/association?edgeId=${encodeURIComponent(edgeId)}`);const data=await response.json();if(!response.ok){inspector.innerHTML=`<p class="graph-error">${safe(data.error||'Evidence unavailable.')}</p>`;return;}const edge=data.association,evidence=edge.properties?.evidence||{},provenance=edge.properties?.provenance||{};inspector.innerHTML=`<div class="eyebrow">Association evidence</div><h3>${safe(edge.sourceLabel)} → ${safe(edge.targetLabel)}</h3><dl><dt>Relation</dt><dd>${safe(edge.kind)}</dd><dt>Weight</dt><dd>${Number(edge.weight).toFixed(3)}</dd><dt>Method</dt><dd>${safe(evidence.method||'—')}</dd><dt>Count</dt><dd>${safe(evidence.count??'—')}</dd><dt>Source</dt><dd>${safe(provenance.source||'—')}</dd><dt>Generated</dt><dd>${safe(provenance.generatedAt||edge.createdAt||'—')}</dd></dl>${evidence.description?`<p>${safe(evidence.description)}</p>`:''}`;});
          ...
          slider('Stability',.35,.9,stability,.05,(value) => { stability=value;graph.d3VelocityDecay(value); });
          ...
          toggle('Concept clouds',true,(checked) => { showConceptClouds=checked;graph?.refresh?.(); });
          ...
          ([['Papers','paper'],['Authors','author'],['Topics','topic'],['Venues','venue'],['Institutions','institution'],['Collections','collection']] as Array<[string,string]>).forEach(([label,key]) => toggle(label,enabled[key]??true,(checked) => { enabled[key]=checked; update(); }));
          if(globalGraph)toggle('Emerging language',enabled.emergence,(checked)=>{enabled.emergence=checked;update();});
          ...
          button('Reset',()=>{focusId=null;hierarchyRootId=null;hoverGraphNode(null);collapsed.clear();selectedNodeIds.clear();activeDiscoveryNode=null;renderSelection();resetDiscoveryPanels();searchInput.value='';graphQuery='';update();graph.zoomToFit(350,40);});
          if(!globalGraph){const refreshSources=button('Refresh current paper graph',()=>{void (async()=>{if(!currentGraphReferenceId)return;refreshSources.disabled=true;refreshSources.textContent='Refreshing…';try{const response=await fetch('/api/knowledge-graph/expand',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({paperId:currentGraphReferenceId})});const result=await response.json().catch(()=>({}));if(!response.ok)throw new Error(result.error||'Could not refresh graph sources.');await loadReferenceGraph(currentGraphReferenceId);refreshSources.textContent=result.warning==='openalex_not_configured'?'Bibliography refreshed':'Paper graph refreshed';}catch(error){refreshSources.textContent=error instanceof Error?error.message:'Refresh failed';}finally{refreshSources.disabled=false;}})();});}
          ...
          loadReferenceGraph=async(nextReferenceId:string)=>{currentGraphReferenceId=nextReferenceId;const nextReference=references.get(nextReferenceId);count.textContent='Loading…';graphKind.textContent='Document graph';setInlineTitle(graphScope,nextReference?.title||'Unknown document');const response=await fetch(`/api/knowledge-graph?paperId=${encodeURIComponent(nextReferenceId)}`,{cache:'no-store'});const next=await response.json();if(!response.ok)throw new Error(next.error||'Could not load graph.');allNodes.splice(0,allNodes.length,...(next.nodes||[]).map((node:any)=>({...node,classification:node.kind||'automatic'})));allLinks.splice(0,allLinks.length,...(next.edges||[]).map((edge:any,index:number)=>({...edge,id:edge.id||`auto-edge:${index}`})));addLocalKeywordSeeds(nextReferenceId);focusId=null;hierarchyRootId=null;hoverGraphNode(null);collapsed.clear();selectedNodeIds.clear();activeDiscoveryNode=null;renderSelection();resetDiscoveryPanels();inspector.innerHTML=next.focus?.found===false?'<div class="eyebrow">Local graph</div><p>Vault keywords are available. Resolve or refresh this paper to add citations and OpenAlex topics.</p>':'<p>Select a paper or association to inspect its evidence.</p>';update();window.requestAnimationFrame(()=>graph.zoomToFit(350,40));};
          loadGlobalGraph=async(collectionId='')=>{count.textContent='Loading…';const collection=payload.libraries.find((library)=>library.id===collectionId);graphKind.textContent='Knowledge graph';graphScope.textContent=collection?.name||'All references';const query=new URLSearchParams();if(collectionId)query.set('collectionId',collectionId);const response=await fetch(`/api/knowledge-graph/corpus?${query}`,{cache:'no-store'});const next=await response.json();if(!response.ok)throw new Error(next.error||'Could not load knowledge graph.');allNodes.splice(0,allNodes.length,...(next.nodes||[]).map((node:any)=>({...node,classification:node.kind||'automatic'})));allLinks.splice(0,allLinks.length,...(next.edges||[]).map((edge:any,index:number)=>({...edge,id:edge.id||`corpus-edge:${index}`})));focusId=null;hierarchyRootId=null;hoverGraphNode(null);collapsed.clear();selectedNodeIds.clear();activeDiscoveryNode=null;renderSelection();resetDiscoveryPanels();inspector.innerHTML=next.nodes?.length?'<p>Select a concept, item, author or emerging phrase to inspect its connected corpus.</p>':'<div class="eyebrow">Empty scope</div><p>This collection has no concepts yet.</p>';update();window.requestAnimationFrame(()=>graph.zoomToFit(350,40));};
  ...
  const stageMessage: Record<string, string> = {
    ...
    scholarly: 'Resolving OpenAlex metadata and rebuilding scholarly links',
  ...
  const runPipelineFollower = async (referenceId: string, activityId: string, filename: string) => {
    ...
    try {
      for (let attempt = 0; attempt < 90; attempt += 1) {
        ...
        if (status.ready) {
          updateActivity(activityId, { state: 'complete', message: `${status.reference.title} · ${activityId.startsWith('scholarly-')?'OpenAlex metadata and graph ready':'metadata and summary ready'}`, referenceId, mapReady: status.reference.hasStructure });
          return;
        }
  ...
  const openHelp = () => {
    ...
    const addShortcutTable=(rows:Array<[string,string]>)=>{const section=document.createElement('section');section.className='help-shortcuts';const heading=document.createElement('h3');heading.textContent='Shortcuts';const table=document.createElement('table');table.className='help-shortcut-table';const tbody=document.createElement('tbody');rows.forEach(([shortcut,action])=>{const row=document.createElement('tr');const key=document.createElement('th');key.scope='row';key.textContent=shortcut;const description=document.createElement('td');description.textContent=action;row.append(key,description);tbody.appendChild(row);});table.appendChild(tbody);section.append(heading,table);body.appendChild(section);};
  ...
  document.addEventListener('keydown', (event) => {
    ...
    if (!editing && event.ctrlKey && !event.metaKey && !event.altKey && (event.key === 'ArrowUp' || event.key === 'ArrowDown')) {
      event.preventDefault();
      const all=event.shiftKey;const collapse=all?event.key==='ArrowUp':event.key==='ArrowDown';
      ...
      else{setSaveState('select a collection first','error');return;}
      window.localStorage.setItem(TREE_STATE_KEY,JSON.stringify([...collapsedLibraries]));renderTree(search.value);
      setSaveState(all?(collapse?'all collections folded':'all collections unfolded'):(collapse?'current collection folded':'current collection unfolded'));
      return;
    }
  ...
  const setActivityConsoleOpen=(open:boolean) => {
    ...
    consoleHeaderToggle?.setAttribute('aria-expanded',String(open));
    consoleHeaderToggle?.classList.toggle('is-active',open);
  ...
  if(consoleResizeHandle){
    setConsoleHeight(Number(window.localStorage.getItem(CONSOLE_HEIGHT_KEY))||Math.min(420,window.innerHeight*.52),false);
    consoleResizeHandle.addEventListener('pointerdown',(event)=>{event.preventDefault();const startY=event.clientY,startHeight=consoleDrawer.getBoundingClientRect().height;consoleResizeHandle.setPointerCapture(event.pointerId);root.classList.add('resizing-console');const move=(moveEvent:PointerEvent)=>setConsoleHeight(startHeight+startY-moveEvent.clientY);const stop=(stopEvent:PointerEvent)=>{root.classList.remove('resizing-console');consoleResizeHandle.releasePointerCapture(stopEvent.pointerId);consoleResizeHandle.removeEventListener('pointermove',move);consoleResizeHandle.removeEventListener('pointerup',stop);consoleResizeHandle.removeEventListener('pointercancel',stop);};consoleResizeHandle.addEventListener('pointermove',move);consoleResizeHandle.addEventListener('pointerup',stop);consoleResizeHandle.addEventListener('pointercancel',stop);});
    consoleResizeHandle.addEventListener('keydown',(event)=>{if(event.key!=='ArrowUp'&&event.key!=='ArrowDown')return;event.preventDefault();setConsoleHeight(consoleDrawer.getBoundingClientRect().height+(event.key==='ArrowUp'?24:-24));});
    window.addEventListener('resize',()=>setConsoleHeight(consoleDrawer.getBoundingClientRect().height,false));
  }
