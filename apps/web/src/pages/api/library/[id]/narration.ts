...
export const POST: APIRoute = async ({ request, locals, params, url }) => {
  ...
  if (input.length < 44 || input.length > 32 * 1024 * 1024) return Response.json({ error: 'invalid_audio_segment' }, { status: 400 });
  ...
  try {
    ...
    const bytes = await readFile(ogg); const bucket = getWasabiBucket();
    ...
    const objectKey = `${storageRoot}/.seshat/${reference.id}/narration/kokoro/${language}-${voice}/${String(segment).padStart(4,'0')}.ogg`;
    ...
    await catalog.pool.query(
      `INSERT INTO catalog_artifacts (id,reference_id,kind,provider,object_key,bucket,mime_type,size_bytes,sha256,etag)
       VALUES($1,$2,'kokoro-audio','wasabi',$3,$4,'audio/ogg; codecs=opus',$5,$6,$7)
      ...
      [randomUUID(),reference.id,objectKey,bucket,bytes.length,sha256,stored.ETag?.replaceAll('"','') || null],
    );
...
export const DELETE: APIRoute = async ({ locals, params }) => {
  ...
  const ownerKey=ownerKeyFor(email);const catalog=getCatalog();const reference=await catalog.get(ownerKey,params.id||'');
  if(!reference||reference.access!=='owner')return Response.json({error:'not_found'},{status:404});
  const bucket=getWasabiBucket();const artifacts=reference.artifacts.filter((item)=>item.kind==='kokoro-audio'&&item.bucket===bucket);
  ...
  await catalog.pool.query(`DELETE FROM catalog_artifacts artifact USING catalog_references reference WHERE artifact.reference_id=reference.id AND reference.id=$1 AND reference.owner_key=$2 AND artifact.kind='kokoro-audio'`,[reference.id,ownerKey]);
