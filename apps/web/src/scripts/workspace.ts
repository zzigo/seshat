  ...
  const toolRenderer = (kind: ToolKind, referenceId?: string, panelId?: string): IContentRenderer => {
    ...
    return { element, init() {
      ...
      if (kind === 'graph') {
        ...
        const conceptSidebar = document.createElement('aside'); conceptSidebar.className = 'graph-concepts';
        ...
        body.append(stage,conceptSidebar,inspector); element.appendChild(body);
        ...
        void fetch(url).then((response) => response.json()).then((data) => {
          ...
          const renderConcepts=()=>{const concepts=allNodes.filter((node:any)=>conceptKinds.has(nodeKind(node))).map((node:any)=>({node,count:degreeCache.get(String(node.id))||0})).sort((left,right)=>right.count-left.count||String(left.node.label).localeCompare(String(right.node.label)));conceptSidebar.replaceChildren();const heading=document.createElement('div');heading.className='eyebrow';heading.textContent=`CONCEPTS — ${concepts.length}`;conceptSidebar.appendChild(heading);const list=document.createElement('div');list.className='graph-concept-list';concepts.forEach(({node,count})=>{const control=document.createElement('button');control.type='button';const label=document.createElement('span');label.textContent=String(node.label||'Untitled concept');const total=document.createElement('b');total.textContent=String(count);control.append(label,total);control.onclick=()=>{layoutMode='concepts';layout.value='concepts';focusId=String(node.id);neighborDepth=Math.max(1,neighborDepth);update();renderConceptNode(node);graph.centerAt(Number(node.x)||0,Number(node.y)||0,400);graph.zoom(2.2,400);};list.appendChild(control);});conceptSidebar.appendChild(list);};
