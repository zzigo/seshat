  ...
  const toolRenderer = (kind: ToolKind, referenceId?: string, panelId?: string): IContentRenderer => {
    ...
    return { element, init() {
      ...
      if (kind === 'graph') {
        ...
        const referencesSection=section('References',true);
        ...
        const citationsSection=section('Citations',false);
        ...
        void fetch(url).then((response) => response.json()).then((data) => {
          ...
          let neighborDepth = 0; let folding = false; let minimumWeight = 0; let graphQuery = ''; let layoutMode: 'relevance' | 'concepts' = 'relevance';const selectedNodeIds=new Set<string>();const discoveryRequests:Record<'references'|'citations'|'similar',number>={references:0,citations:0,similar:0};let activeDiscoveryNode:any=null;
          ...
          const selectGraphNode=(node:any,event?:MouseEvent)=>{const id=String(node.id);const multiple=Boolean(event?.shiftKey||event?.metaKey||event?.ctrlKey);if(!multiple)selectedNodeIds.clear();if(multiple&&selectedNodeIds.has(id))selectedNodeIds.delete(id);else selectedNodeIds.add(id);focusId=id;if(folding){if(collapsed.has(id))collapsed.delete(id);else collapsed.add(id);}renderSelection();update();graph?.refresh?.();if(!selectedNodeIds.has(id))return;if(conceptKinds.has(nodeKind(node)))void unfoldConcept(node);else if(paperKinds.has(nodeKind(node)))void renderPaper(node);else{activeDiscoveryNode=null;resetDiscoveryPanels();}};
          ...
          const drawNode=(node:any,ctx:CanvasRenderingContext2D,scale:number)=>{const x=Number(node.x)||0,y=Number(node.y)||0,kind=normalizedNodeKind(node),radius=nodeRadius(node),selected=selectedNodeIds.has(String(node.id));ctx.save();if(selected){ctx.shadowColor='#d48738';ctx.shadowBlur=Math.max(8,13/Math.max(scale,.4));}ctx.beginPath();if(['concept','topic','relatedconcept'].includes(kind))ctx.rect(x-radius,y-radius,radius*2,radius*2);else if(['paper','work','document','publication','article','ebook'].includes(kind))polygon(ctx,x,y,radius*1.25,4,0);else if(['institution','organization','publisher','venue','journal'].includes(kind))polygon(ctx,x,y,radius*1.15,6);else if(['place','location','geographicregion'].includes(kind))polygon(ctx,x,y,radius*1.2,3);else if(['method','instrument','system'].includes(kind))polygon(ctx,x,y,radius*1.15,5);else ctx.arc(x,y,radius,0,Math.PI*2);ctx.fillStyle=nodeColor(node);ctx.fill();ctx.shadowBlur=0;ctx.lineWidth=Math.max(.7,1/Math.max(scale,.2));ctx.strokeStyle='rgba(20,30,25,.72)';ctx.stroke();if(selected){ctx.beginPath();ctx.arc(x,y,radius+5,0,Math.PI*2);ctx.lineWidth=Math.max(2.2,3/Math.max(scale,.3));ctx.strokeStyle='#d48738';ctx.stroke();}if(kind==='collection'){ctx.beginPath();ctx.arc(x,y,radius*.55,0,Math.PI*2);ctx.stroke();}const lines=nodeLabelLines(node);if(!lines.length){ctx.restore();return;}let vectorX=0,vectorY=0;for(const neighborId of neighborCache.get(String(node.id))||[]){const neighbor=nodeCache.get(neighborId);if(!neighbor)continue;const dx=Number(neighbor.x)-x,dy=Number(neighbor.y)-y,length=Math.hypot(dx,dy)||1;vectorX+=dx/length;vectorY+=dy/length;}const placement=graphLabelPlacement(vectorX,vectorY);const fontSize=Math.max(7,Math.min(11,10/Math.max(.75,scale)));const lineHeight=fontSize*1.18,paddingX=Math.max(2,3/Math.max(.75,scale)),paddingY=Math.max(2,2.5/Math.max(.75,scale));ctx.font=`500 ${fontSize}px "Helvetica Neue",Helvetica,Arial,sans-serif`;const width=Math.max(...lines.map((line)=>ctx.measureText(line).width))+paddingX*2,height=lines.length*lineHeight+paddingY*2,gap=Math.max(4,5/Math.max(.75,scale));let boxX=x-width/2,boxY=y+radius+gap;if(placement==='above')boxY=y-radius-gap-height;else if(placement==='left'){boxX=x-radius-gap-width;boxY=y-height/2;}else if(placement==='right'){boxX=x+radius+gap;boxY=y-height/2;}const styles=getComputedStyle(document.documentElement),paper=styles.getPropertyValue('--paper').trim()||'#f1efe6',ink=styles.getPropertyValue('--ink').trim()||'#263129',line=styles.getPropertyValue('--line').trim()||'rgba(20,30,25,.22)';ctx.globalAlpha=.96;ctx.fillStyle=paper;ctx.fillRect(boxX,boxY,width,height);ctx.globalAlpha=1;ctx.strokeStyle=selected?'#d48738':line;ctx.lineWidth=selected?Math.max(1.6,2/Math.max(scale,.3)):Math.max(.45,.7/Math.max(scale,.3));ctx.strokeRect(boxX,boxY,width,height);ctx.fillStyle=ink;ctx.textAlign='center';ctx.textBaseline='middle';lines.forEach((text,index)=>ctx.fillText(text,boxX+width/2,boxY+paddingY+lineHeight*(index+.5)));ctx.restore();};
          graph = new ForceGraph(stage)
            ...
            .onBackgroundClick(()=>{selectedNodeIds.clear();focusId=null;activeDiscoveryNode=null;renderSelection();resetDiscoveryPanels();inspector.innerHTML='<p>Select a paper or association to inspect its evidence.</p>';update();graph?.refresh?.();})
          ...
          button('Reset',()=>{focusId=null;collapsed.clear();selectedNodeIds.clear();activeDiscoveryNode=null;renderSelection();resetDiscoveryPanels();searchInput.value='';graphQuery='';update();graph.zoomToFit(350,40);});
