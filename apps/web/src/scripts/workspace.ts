    ...
    return { element, init() {
      ...
      if (kind === 'graph') {
        ...
        void fetch(url).then((response) => response.json()).then((data) => {
          ...
          let neighborDepth = 0; let folding = false; let minimumWeight = 0; let graphQuery = ''; let layoutMode: 'relevance' | 'concepts' = 'relevance';const selectedNodeIds=new Set<string>();let referenceRequest=0;
          ...
          const mergeGraphData=(nodes:any[],edges:any[])=>{const nodeMap=new Map(allNodes.map((node:any)=>[String(node.id),node]));nodes.forEach((node:any)=>{const current=nodeMap.get(String(node.id));nodeMap.set(String(node.id),current?{...current,...node,properties:{...(current.properties||{}),...(node.properties||{})}}:node);});const edgeMap=new Map(allLinks.map((edge:any)=>[String(edge.id),edge]));edges.forEach((edge:any)=>edgeMap.set(String(edge.id),edge));allNodes.splice(0,allNodes.length,...nodeMap.values());allLinks.splice(0,allLinks.length,...edgeMap.values());};
          ...
          graph = new ForceGraph(stage)
            ...
            .onBackgroundClick(()=>{selectedNodeIds.clear();focusId=null;renderSelection();referenceRequest+=1;referencesSection.badge.textContent='';referenceSidebar.innerHTML='<p>Select a paper to inspect its references.</p>';inspector.innerHTML='<p>Select a paper or association to inspect its evidence.</p>';update();graph?.refresh?.();})
          ...
          button('Reset',()=>{focusId=null;collapsed.clear();selectedNodeIds.clear();renderSelection();referenceRequest+=1;referencesSection.badge.textContent='';referenceSidebar.innerHTML='<p>Select a paper to inspect its references.</p>';searchInput.value='';graphQuery='';update();graph.zoomToFit(350,40);});
