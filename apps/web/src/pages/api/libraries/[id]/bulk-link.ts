...
import { getWasabiLibraryRoot } from '../../../../lib/wasabi-settings';

const supported=/\.(pdf|epub|docx|txt)$/i;const MAX_OBJECTS=100_000;
const cleanSegment=(value:unknown):string=>String(value||'').normalize('NFC').replace(/[\u0000-\u001f\u007f/\\]/g,'').trim();
const normalizedPath=(value:string):string=>value.normalize('NFKD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/\/{2,}/g,'/').replace(/^\/+|\/+$/g,'');
...
export const GET:APIRoute=async({locals,params})=>{
  ...
  const identity={email,name:String(user?.name||'')};const root=await getWasabiLibraryRoot(ownerKey,identity);const folder=pathFor(target.id);const prefix=folder?`${root}/${folder}/`:`${root}/`;
  const storage=getWasabiClient(),bucket=getWasabiBucket(),objects:WasabiMatchObject[]=[];let token:string|undefined;
  do{const page=await storage.send(new ListObjectsV2Command({Bucket:bucket,Prefix:prefix,ContinuationToken:token,MaxKeys:1000}));for(const object of page.Contents||[]){const key=String(object.Key||''),filename=key.split('/').at(-1)||'';if(key&&filename&&supported.test(filename)&&!key.includes('/.seshat/'))objects.push({key,filename,path:key.slice(root.length+1),sizeBytes:Number(object.Size||0),lastModified:object.LastModified?.toISOString()});if(objects.length>=MAX_OBJECTS)break;}token=objects.length>=MAX_OBJECTS?undefined:page.NextContinuationToken;}while(token);
  ...
  const available=objects.filter((object)=>!linkedKeys.has(object.key));const byFolder=new Map<string,WasabiMatchObject[]>();available.forEach((object)=>{const directory=object.path.includes('/')?object.path.slice(0,object.path.lastIndexOf('/')):'';const key=normalizedPath(directory);byFolder.set(key,[...(byFolder.get(key)||[]),object]);});
