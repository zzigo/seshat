  ...
  const toolRenderer = (kind: ToolKind, referenceId?: string, panelId?: string): IContentRenderer => {
    ...
    return { element, init() {
      ...
      if (kind === 'graph') {
        ...
        const sidebarOpen=document.createElement('button'); sidebarOpen.type='button'; sidebarOpen.className='graph-sidebar-open'; sidebarOpen.title='Show graph sidebar'; sidebarOpen.setAttribute('aria-label','Show graph sidebar'); sidebarOpen.innerHTML='<svg viewBox="0 0 16 16" aria-hidden="true"><rect x="2" y="2" width="12" height="12" rx="1"/><path d="M10 2v12"/></svg>';
