    ...
    return { element, init() {
      ...
      if (kind === 'graph') {
        ...
        void fetch(url).then((response) => response.json()).then((data) => {
          ...
          const drawConceptClouds=(ctx:CanvasRenderingContext2D,scale:number)=>{if(!showConceptClouds||!renderedNodes.length)return;const visibleById=new Map(renderedNodes.map((node:any)=>[String(node.id),node]));for(const concept of renderedNodes.filter((node:any)=>conceptKinds.has(nodeKind(node)))){const conceptId=String(concept.id),members=new Map<string,any>([[conceptId,concept]]);for(const link of renderedLinks){const source=sourceId(link),target=targetId(link);if(source!==conceptId&&target!==conceptId)continue;const neighbor=visibleById.get(source===conceptId?target:source);if(neighbor&&paperKinds.has(nodeKind(neighbor)))members.set(String(neighbor.id),neighbor);}const positions=[...members.values()].map((node:any)=>({x:Number(node.x),y:Number(node.y)})).filter((position)=>Number.isFinite(position.x)&&Number.isFinite(position.y));if(positions.length<2)continue;const minX=Math.min(...positions.map((position)=>position.x)),maxX=Math.max(...positions.map((position)=>position.x)),minY=Math.min(...positions.map((position)=>position.y)),maxY=Math.max(...positions.map((position)=>position.y)),cx=(minX+maxX)/2,cy=(minY+maxY)/2,padding=Math.max(28,46/Math.max(.55,scale)),rx=Math.max(48,(maxX-minX)*.63+padding),ry=Math.max(38,(maxY-minY)*.63+padding),index=conceptKikiBoubaIndex(concept.label,concept.properties||{}),color=conceptCloudColor(concept);ctx.save();conceptCloudPath(ctx,cx,cy,rx,ry,index);ctx.globalAlpha=.13;ctx.fillStyle=color;ctx.fill();ctx.globalAlpha=.3;ctx.strokeStyle=color;ctx.lineWidth=Math.max(.7,1.1/Math.max(.5,scale));ctx.stroke();conceptCloudPath(ctx,cx,cy,rx,ry,index,.72);ctx.globalAlpha=.18;ctx.setLineDash([Math.max(2,4/Math.max(.5,scale)),Math.max(2,5/Math.max(.5,scale))]);ctx.stroke();ctx.setLineDash([]);if(scale>.42){ctx.globalAlpha=.38;ctx.fillStyle=color;ctx.font=`600 ${Math.max(7,9/Math.max(.8,scale))}px "Helvetica Neue",Helvetica,Arial,sans-serif`;ctx.textAlign='center';ctx.textBaseline='bottom';ctx.fillText(plainInlineTitle(concept.label||''),cx,cy-ry+Math.max(10,13/Math.max(.7,scale)));}ctx.restore();}};
