...
type MobileItem={id:string;title:string;type:string;persons:string;year:number|null;language:string;format:string;hasOriginal:boolean;hasOpenAlex:boolean;hasAnnotations:boolean;hasText:boolean;hasStructure:boolean;sizeBytes:number;progress:number;readAt:string|null;updatedAt:string};
...
export const mountMobwork=(root:HTMLElement)=>{
  ...
  const renderWindow=()=>{
    ...
    for(let index=start;index<end;index+=1){
      ...
      const format=document.createElement('span');format.className='mobwork-row-format';const itemKind=bibliographicVisualKind(item.type),fileKind=referenceVisualKind(item.format,item.hasText),linkState=referenceLinkState(item),processes=referenceProcessKinds(item);format.title=[bibliographicVisualLabel(itemKind),linkState==='unlinked'?'No associated file':item.format.toUpperCase(),...processes.map(referenceProcessLabel)].filter(Boolean).join(' · ');const glyph=document.createElement('i');glyph.className=`tree-reference-glyph item-${itemKind} is-${linkState}`;glyph.dataset.fileKind=fileKind;if(processes.length){const dots=document.createElement('span');dots.className='tree-process-dots';dots.ariaHidden='true';processes.forEach((process)=>{const dot=document.createElement('i');dot.className=`tree-process-dot is-${process}`;dots.appendChild(dot);});glyph.appendChild(dots);}format.appendChild(glyph);
