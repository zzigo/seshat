    ...
    return { element, init() {
      ...
      if (kind === 'graph') {
        ...
        void fetch(url).then((response) => response.json()).then((data) => {
          ...
          const nodeLabelLines=(node:any)=>{const label=plainInlineTitle(node.label||'');if(!paperKinds.has(nodeKind(node)))return wrapGraphLabel(label,24,3);const {author,year}=paperMeta(node);if(hoveredNodeId===String(node.id))return[...wrapGraphLabel(label,30,8),`${author} · ${year}`];return[author,year];};
