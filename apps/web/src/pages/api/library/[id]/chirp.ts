...
export const POST:APIRoute=async({request,locals,params})=>{
  ...
  const body=await request.json().catch(()=>null),text=String(body?.text||'').trim(),ssml=String(body?.ssml||'').trim(),voice=chirpVoice(String(body?.voice||''));
  const validSsml=!ssml||(/^<speak>[\s\S]*<\/speak>$/i.test(ssml)&&!/<(?!\/?(?:speak|s|p|break)(?:\s|\/?>))[^>]+>/i.test(ssml));
  const input=ssml||text,characters=billableCharacterCount(input);
  if(!text||!validSsml||characters>4500)return Response.json({error:'Text or structural SSML must contain between 1 and 4,500 characters.'},{status:400});
  if(!voice)return Response.json({error:'Unsupported Chirp voice.'},{status:400});
  const rate=Math.max(.5,Math.min(2,Number(body?.rate||1))),month=chirpMonth(),{limit,warning}=limits();
  ...
  try{
    ...
    const [result]=await client.synthesizeSpeech({input:ssml?{ssml}:{text},voice:{languageCode:voice.id.slice(0,5),name:voice.id},audioConfig:{audioEncoding:'OGG_OPUS',speakingRate:rate}});
