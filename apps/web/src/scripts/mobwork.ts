...
type MobilePayload={libraries:MobileLibrary[]};
...
export const mountMobwork=(root:HTMLElement)=>{
  ...
  const renderWindow=()=>{
    ...
    for(let index=start;index<end;index+=1){
      ...
      const format=document.createElement('span');format.className='mobwork-row-format';format.textContent=(item.format||item.type||'doc').slice(0,4).toUpperCase();
